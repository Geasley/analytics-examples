...
class Valuation(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.configuration_recipe_api = lusid.ConfigurationRecipeApi(cls.api_client)
    ...
    def test_portfolio_aggregation(self):
        ...
        portfolio_code = self.transaction_portfolios_api.create_transaction_portfolio(TestDataUtilities.tutorials_scope)
    ...
    # and there is nothing in NextEvent, because known cashflows are not regarded as events
    # Models tested - LUSID
    # 1 LUSID aggregation in inline, using weighted instruments which are not persisted
    # 2 LUSID aggregation vs portfolio, upserted recipe and instruments
    ...
    def test_IRS_schedule(self):
        ...
        tradingCode = "Global-Equity-swap"

        trade_date = datetime(2020, 1, 15, tzinfo=pytz.utc) #datetime.today().replace(tzinfo=pytz.utc)
        start_date = trade_date
        effectiveAt = start_date
        ...
        swapInstrumentDefinition = models.LusidInstrumentDefinition(
            name="10yEURIRS",
        ...
        upsertResponse = self.instruments_api.upsert_lusid_instruments(instruments=batchUpsertRequest)

        # now we need to book a transaction
        ...
        self.transaction_portfolios_api.upsert_transactions(scope=tradingScope,
                                                            code=tradingCode,
        ...
        RecipeId = models.ConfigurationRecipe(
            code="SwapExampleCashflowRecipe",
            pricing=pricingContext,
            market=marketContext,
            description="Test swap cashflow recipe"
        )
        # upsert the recipe
        response = self.configuration_recipe_api.upsert_configuration_recipe(
            scope="Finbourne-Examples",
            structured_data=models.UpsertRecipeRequest(code="FinbourneExamplesSwapCashflow",
                                                       configuration_recipe=RecipeId)
        )
        ...
        inlineRequestIRS = models.InlineAggregationRequest(request=aggregationRequestResource,
                                                             instruments=[weightedInstrument_10yEURIRS])

        # Call LUSID to perform the aggregation

        inlineResponse = self.aggregation_api.get_aggregation_of_weighted_instruments(marketDataScope,
                                                                                inline_request=inlineRequestIRS)

        response = self.aggregation_api.get_aggregation_by_portfolio(
            scope=tradingScope,
            code=tradingCode,
            request=aggregationRequestResource)
    ...
    def test_recipe_order(self):
        ...
        # create a LUSID instrument definition, put swap in, create instrument
        # then upsert OTC instruments
    ...
    def test_CDS(self):
        ...
        vendorModel = models.VendorModelRule(supplier="Lusid", model_name="VendorDefault",
                                                  instrument_type="CreditDefaultSwap", parameters="{}")
    ...
    ################################################
    # Test: create a payer swaption
    # Tenor: 10y, option 3m
    # 75mil EUR IRS underlying
    # Test will show underlying cashflows
    #
    #
    # Models tested - LUSID
    # LUSID aggregation in inline, using weighted instruments which are not persisted
    ...
    def test_SWOPT_expiry_and_exercise(self):
        ...
        # we create a 3m->10y swaption
        trade_date = datetime.today().replace(tzinfo=pytz.utc)
        option_expiry_date = trade_date.replace(month=trade_date.month + 3)
        swap_start_date = option_expiry_date
        end_date_10y = swap_start_date.replace(year=swap_start_date.year + 10)
        ...
        # create the swap legs themselves..pay fixed rate of 1%
        ...
        #create the swap
        instrument_definition_10yEURIRS = models.SwapInstrument(
            start_date=swap_start_date.isoformat(),
            maturity_date=end_date_10y.isoformat(),
            notional=dom_amount,
            legs=irs_legs,
            is_amortizing=False,
        ...
        # we create a physical delivery and a cash delivery swaption
        # based on the same underlying swap

        swopt_inst_phys = models.Swaption(
            start_date=option_expiry_date.isoformat(),
            is_payer_not_receiver=True,
            is_delivery_not_cash=True,
            swap=instrument_definition_10yEURIRS,
            instrument_type="InterestRateSwaption"
        )

        swopt_inst_cash = models.Swaption(
            start_date=option_expiry_date.isoformat(),
            is_payer_not_receiver=True,
        ...
        weightedInstrument_10yEURSWOPTCASH = models.WeightedInstrument(quantity=1, holding_identifier="myholding_10yEURSWOPTCASH",
                                                                 instrument=swopt_inst_cash)
        weightedInstrument_10yEURSWOPTPHY = models.WeightedInstrument(quantity=1,
                                                                       holding_identifier="myholding_10yEURSWOPTPHY",
                                                                       instrument=swopt_inst_phys)
        aggregationRequestResource = models.AggregationRequest(
            inline_recipe=RecipeId,
            ...
            metrics=[
                models.AggregateSpec(key='Analytic/default/ValuationDate',
                                     op='Value'),
                models.AggregateSpec(key='Analytic/default/OnExercise',
                                     op='Value'),
                models.AggregateSpec(key='Analytic/default/NextEvent',
                                     op='Value'),
                models.AggregateSpec(key='Analytic/default/NextEventType',
        ...
        inlineRequestSWOPTCASH = models.InlineAggregationRequest(request=aggregationRequestResource,
                                                             instruments=[weightedInstrument_10yEURSWOPTCASH])
        inlineRequestSWOPTPHY = models.InlineAggregationRequest(request=aggregationRequestResource,
                                                                 instruments=[weightedInstrument_10yEURSWOPTPHY])

        # Call LUSID to perform the aggregation

        responseCASH= self.aggregation_api.get_aggregation_of_weighted_instruments(marketDataScope,
                                                                                inline_request=inlineRequestSWOPTCASH)

        responsePHY = self.aggregation_api.get_aggregation_of_weighted_instruments(marketDataScope,
                                                                                    inline_request=inlineRequestSWOPTPHY)


        #cash has a cashflow in 3months as you'd expect. there is no swap since we are settling cash
        print(responseCASH)

        # the physically settled swaptio can be interrogated for the underlying IRS
        # in future versions when the model is connected, the user will be able to choose the p weighted flows
        print(responsePHY)
        #for the underlying swap information:
        print(responsePHY.data[0]['Analytic/default/OnExercise']['information'])
        print(responsePHY.data[0]['Analytic/default/HoldingCashflows'])




    ###############################################
    # Test to ensure we can save down some recipes.
    # We then run aggregations to show the difference in result
    def test_createSomeRecipes(self):
        # Specify the recipe used to perform the aggregation
        config_recipe = models.ConfigurationRecipe(
            code='FinbourneExamplesRecipeMidThenBid',
            market=models.MarketContext(
                market_rules=[
                    models.MarketDataKeyRule(
                        key='Equity.Figi.*',
            ...
            description="For each instrument (Equity and Fx) use Mid Price if available, Bid if not"
            )

        #upsert the recipe
        response = self.configuration_recipe_api.upsert_configuration_recipe(
            scope="Finbourne-Examples",
            structured_data=models.UpsertRecipeRequest(code="FinbourneExamplesRecipeMidThenBid",
                                                       configuration_recipe=config_recipe)
        )
        config_recipe = models.ConfigurationRecipe(
            code='FinbourneExamplesRecipeBidThenMid',
            market=models.MarketContext(
                market_rules=[
                    models.MarketDataKeyRule(
                        key='Equity.Figi.*',
            ...
            description="For each instrument (Equity and Fx) use Bid Price if available, Mid if not"
        )

        # upsert the recipe
        response = self.configuration_recipe_api.upsert_configuration_recipe(
            scope="Finbourne-Examples",
            structured_data=models.UpsertRecipeRequest(code="FinbourneExamplesRecipeBidThenMid",
                                                       configuration_recipe=config_recipe)
        )
        config_recipe = models.ConfigurationRecipe(
            code='FinbourneExamplesRecipeLusidFlatFX',
            market=models.MarketContext(
                market_rules=[
                    models.MarketDataKeyRule(
                        key='Equity.Figi.*',
            ...
            description = "Change supplier to Lusid for FX, to show an arbitrary constant FX rate"
        )

        # upsert the recipe
        response = self.configuration_recipe_api.upsert_configuration_recipe(
            scope="Finbourne-Examples",
            structured_data=models.UpsertRecipeRequest(code="FinbourneExamplesRecipeLusidFlatFX",
                                                       configuration_recipe=config_recipe)
        )

        # # create a snippet
        # config_snippet = models.ConfigurationRecipeSnippet(
        #     market_rules=[],
        #     market_options=None
        # )
        trade_date = datetime(2019, 4, 20, tzinfo=pytz.utc)  # datetime.today().replace(tzinfo=pytz.utc)
        start_date = trade_date
        # create an aggregation to test the functioning of this vs the example set
        # Use our saved recipes via the resource ID
        ...
        aggregationRequestResource = models.AggregationRequest(
            ...
            metrics=[
                ...
                models.AggregateSpec(key='Instrument/default/Name',
                                     op='Value'),
                models.AggregateSpec(key='Holding/default/ExchangeRate',
                                     op='Value'),
                models.AggregateSpec(key='Holding/default/PvCcy',
                                     op='Value')
            ],
            group_by=['Instrument/default/Name']
        )
        ...
        #   run the aggregation
        aggregationResponse = self.aggregation_api.get_aggregation_by_portfolio(
            scope='Finbourne-Examples',
            code='Global-Equity',
            request=aggregationRequestResource)
        ...
        #access the manifest
        manifestResponse = aggregationResponse.manifest

        print(aggregationResponse)

    ###############################################
    # Test to create a new vanilla bond
    # and show accrual working
    # By also overwriting the calculated accrual,
    # we also show the unit result store in operation

    def test_bondAccrual(self):

        # we'll use the gilt 1.5% 47s
        # maturity date 22 - Jul - 2047
        # issue date 21 - Sep - 2016
        # coupons 22Jan / Jul
        # next coupon 13 - Jul - 2020
        # issue size 24, 446 (£mil)

        marketDataScope = "FinbourneMarketData"
        trade_date = datetime.today().replace(tzinfo=pytz.utc)
        start_date = datetime(2016, 9, 21, tzinfo=pytz.utc)
        #start_date = datetime(2020, 1, 22, tzinfo=pytz.utc)
        ...
        # create the flow convention

        flow_conventionGilt = models.FlowConventions(
            currency="GBP",
            payment_frequency=models.Tenor(value=6, unit="M"),
        ...
        # create the bond
        instrument_definition_gilt_2047s = models.BondInstrument(
            start_date=start_date.isoformat(),
            maturity_date= maturity_date.isoformat(),
            dom_ccy=domCcy,
            coupon_rate=coupon,
            principal=faceValue,
            flow_conventions=flow_conventionGilt,
        ...
        weightedInstrument_gilt = models.WeightedInstrument(quantity=1, holding_identifier="myholding_gilt",
                                                                 instrument=instrument_definition_gilt_2047s)
        aggregationRequestResource = models.AggregationRequest(
            inline_recipe=RecipeId,
            effective_at=trade_date.isoformat(),
            metrics=[
        ...
        inlineRequestGilt = models.InlineAggregationRequest(request=aggregationRequestResource,
                                                             instruments=[weightedInstrument_gilt])

        # Call LUSID to perform the aggregation

        response = self.aggregation_api.get_aggregation_of_weighted_instruments(marketDataScope,
                                                                                inline_request=inlineRequestGilt)
